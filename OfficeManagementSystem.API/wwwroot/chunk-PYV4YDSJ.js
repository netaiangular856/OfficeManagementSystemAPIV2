...
var DepartmentFormComponent = class _DepartmentFormComponent {
  ...
  setupManagerSearch() {
    this.managerSearchSubject.pipe(debounceTime(300), distinctUntilChanged(), switchMap((searchTerm) => {
      ...
      return this.usersService.searchUserNames(term.length ? term : void 0).pipe(catchError((error) => {
      ...
      this.filteredManagers.set(userList);
      const hasResults = userList.length > 0;
  ...
  loadDropdownData() {
    ...
    if (this.isEdit && this.department && this.department.managerUserId) {
      this.usersService.searchUserNames().subscribe({
        next: (response) => {
          const userList = Array.isArray(response?.data) ? response.data : [];
          const user = userList.find((u) => u.id === this.department.managerUserId);
          if (user) {
            this.selectedManager.set(user);
            this.managerSearchTerm.set(user.name);
          }
        },
        error: (error) => {
...
//# sourceMappingURL=chunk-PYV4YDSJ.js.map
