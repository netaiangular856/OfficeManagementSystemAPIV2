...
var UsersService = class _UsersService {
  ...
  getManagerNames(search) {
    ...
    return this.http.get(`${environment.apiBaseUrl}/Users/manager-names`, {
      headers: this.getAuthHeaders(),
      params: httpParams
    }).pipe(map((response) => {
      if (response.success && Array.isArray(response.data)) {
  ...
  getDirectEmployees() {
    return this.http.get(`${environment.apiBaseUrl}/Employees/names`, { headers: this.getAuthHeaders() }).pipe(map((response) => {
      if (response.success && Array.isArray(response.data)) {
        return response.data.map((employee) => ({
          id: employee.email,
          // Using email as ID since the API doesn't provide a separate ID
          name: employee.fullName,
          departmentName: employee.jobTitle
        }));
      }
      return [];
    }));
...
//# sourceMappingURL=chunk-IC3VYVY3.js.map
